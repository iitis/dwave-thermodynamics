...
for h_val in [0.1, 0.5, 1.0]:
    for tau in ["2.00", "24.00", "46.00", "68.00", "90.00", "112.00", "134.00", "156.00"]:
        ...
        df = pd.read_csv(os.path.join(cwd, f"results\\raw_data\\raw_data_pausing_tau_{tau}_beta_1_h{h_val}_s0.41.csv"),
                             sep=";", index_col=0)
        for init in tqdm(df.init_state.unique().tolist(), desc=f"{tau}, {h_val}"):
        ...
        mean_Q[(tau, h_val)] = (np.mean(np.array(Q)))
        var_E[(tau, h_val)] = (np.var(np.array(E_fin)))
with open("C:\\Users\\tsmierzchalski\\PycharmProjects\\dwave-thermodynamics\\results\\results_pausing_h_q_varE.pkl",
          "wb") as f:
    pickle.dump([var_E, mean_Q], f)
